...
const CATALOG = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'tests/catalog_100.json'), 'utf-8'));
...
async function runOne(entry) {
  ...
  try {
    ...
    try { json = JSON.parse(text); } catch { err = `non-json ${text.slice(0,200)}`; }
  ...
  return {
    ...
    snippet: content.slice(0, 220).replace(/\n/g, ' '),
  };
}
...
async function main() {
  ...
  async function worker() {
    while (idx < CATALOG.length) {
      ...
      console.log(`[${String(r.id).padStart(3,'0')}] ${status} (${r.latency}ms) isLive=${r.meta?.isLive} mode=${r.mode} q="${r.q.slice(0,55)}" ${r.fails.length ? '-> ' + r.fails.join(' | ') : ''}`);
    }
  }
  ...
  fs.writeFileSync(path.join(process.cwd(), 'tests/results_100.json'), JSON.stringify({ summary:{ total: results.length, pass, fail, avgLatency, livePass, liveTotal, fbPass, fbTotal }, results }, null, 2), 'utf-8');
  fs.writeFileSync(path.join(process.cwd(), 'tests/results_100.md'), buildMarkdown(pass, fail, results), 'utf-8');
  console.log('Saved tests/results_100.json + .md');
  const releaseOk = fail === 0;
  console.log('\n=== RELEASE GATE ===');
  if (releaseOk) console.log('✅ READY TO RELEASE — all 100 cases PASS');
...
function buildMarkdown(pass, fail, results) {
  ...
  const fbPass = results.filter(r=>r.mode==='fallback' && r.ok).length;
  const fbTotal = results.filter(r=>r.mode==='fallback').length;
  let md = `# 100 Pertanyaan — Hasil Test rf-copilot\n\n`;
  ...
  for (const r of results) {
    const isu = r.fails.length ? r.fails.join('; ').replace(/\|/g,'/').slice(0,80) : '-';
    md += `| ${r.id} | ${r.mode} | ${r.q.slice(0,60)} | ${r.ok?'PASS':'FAIL'} | ${r.latency}ms | ${isu} |\n`;
  }
  md += `\n## Kriteria Release\n- Semua 100 kasus PASS\n- Live pass ≥95%\n- Tidak ada hardcode Google AI Studio di mode live\n- Fallback human tanpa "Skill aktif" di content\n\n`;
  md += `Hasil: **${fail===0?'READY TO RELEASE ✅':'NOT READY ⛔'}**\n`;
  return md;
}
